...
//    EntityListImpl getEmptyList() { return emptyList }
    ...
    MNode getEntityFacadeNode() { return null } // ecfi.getConfXmlRoot().first("entity-facade") }
...
////        XADataSource xaDs = (XADataSource) MClassLoader.loadClass(xaDsClassName).newInstance()
//        for (Map.Entry<String, String> attrEntry in xaPropsNode.attributes.entrySet()) {
//            String propValue = attrEntry.value //ecfi.resourceFacade.expand(attrEntry.value, "", confMap)
//            try {
    ...
    String formatFieldString(String entityName, String fieldName, String value) {
        ...
        if (fi.typeValue == 2) {
            if (value.matches("\\d*")) {
                ...
                outVal = new Timestamp(Long.parseLong(value)).toString() //ecfi.l10n.format(new Timestamp(Long.parseLong(value)), null)
            }
        } else if (fi.type.startsWith("currency-")) {
            outVal = new BigDecimal(value) //ecfi.l10n.format(new BigDecimal(value), "#,##0.00#")
        }
