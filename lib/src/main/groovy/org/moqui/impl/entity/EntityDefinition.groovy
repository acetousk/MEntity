...
class EntityDefinition {
    ...
    private Map<String, EntityJavaUtil.RelationshipInfo> relationshipInfoMap = null
    private ArrayList<EntityJavaUtil.RelationshipInfo> relationshipInfoList = null
    ...
    EntityJavaUtil.RelationshipInfo getRelationshipInfo(String relationshipName) {
        if (relationshipName == null || relationshipName.isEmpty()) return null
        return getRelationshipInfoMap().get(relationshipName) as EntityJavaUtil.RelationshipInfo
    }
    Map<String, EntityJavaUtil.RelationshipInfo> getRelationshipInfoMap() {
    ...
    private synchronized void makeRelInfoMap() {
        ...
        Map<String, EntityJavaUtil.RelationshipInfo> relInfoMap = new HashMap<String, EntityJavaUtil.RelationshipInfo>()
        List<EntityJavaUtil.RelationshipInfo> relInfoList = getRelationshipsInfo(false)
        for (EntityJavaUtil.RelationshipInfo relInfo in relInfoList) {
    ...
    ArrayList<EntityJavaUtil.RelationshipInfo> getRelationshipsInfo(boolean dependentsOnly) {
        if (relationshipInfoList == null) makeRelInfoList()

        if (!dependentsOnly) return new ArrayList(relationshipInfoList)
        // just get dependents
        ArrayList<EntityJavaUtil.RelationshipInfo> infoListCopy = new ArrayList<>()
        for (EntityJavaUtil.RelationshipInfo info in relationshipInfoList) if (info.dependent) infoListCopy.add(info)
        return infoListCopy
    }
    private synchronized void makeRelInfoList() {
        ...
        ArrayList<EntityJavaUtil.RelationshipInfo> infoList = new ArrayList<>()
        for (MNode relNode in this.expandedRelationshipList) {
            EntityJavaUtil.RelationshipInfo relInfo = new EntityJavaUtil.RelationshipInfo(relNode, this, efi)
    ...
    static class MasterDetail {
        ...
        EntityJavaUtil.RelationshipInfo relInfo
        ...
        MasterDetail(EntityDefinition parentEd, MNode detailNode) {
            ...
            if (relInfo == null) throw new BaseException("Invalid relationship name [${relationshipName}] for entity ${parentEd.getFullEntityName()}")
        ...
        ArrayList<MasterDetail> getDetailList() {
            if (relatedMasterName) {
                ...
                if (relatedMaster == null) throw new BaseException("Invalid use-master value [${relatedMasterName}], master not found in entity ${relInfo.relatedEntityName}")
    ...
    static class EntityDependents {
        ...
        Map<String, EntityJavaUtil.RelationshipInfo> relationshipInfos = new HashMap<String, EntityJavaUtil.RelationshipInfo>()
        ...
        EntityDependents(EntityDefinition ed, Deque<String> ancestorEntities, Map<String, EntityDependents> allDependents) {
            ...
            List<EntityJavaUtil.RelationshipInfo> relInfoList = ed.getRelationshipsInfo(true)
            for (EntityJavaUtil.RelationshipInfo relInfo in relInfoList) {
        ...
        void buildString(StringBuilder builder, int level, Set<String> entitiesVisited) {
            ...
            for (Map.Entry<String, EntityDependents> entry in dependentEntities) {
                EntityJavaUtil.RelationshipInfo relInfo = relationshipInfos.get(entry.getKey())
...
//        return fieldInfo.convertFromString(value)
//    }
